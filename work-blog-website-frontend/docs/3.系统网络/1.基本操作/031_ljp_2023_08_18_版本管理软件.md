...
> 
> ```shell
> git fetch origin  # 拉取 origin/main、origin/dev 所有最新提交（仅缓存，不修改工作区）
> git checkout main # 切换到 main 分支
> git rebase origin/main # 用 rebase 同步远端 main
> # 打开冲突文件，删除 <<<<<<</=======/>>>>>>> 标记，手动合并内容
> # 标记冲突已解决：git add 冲突文件名
> # 继续变基：git rebase --continue
> # 重复直到无冲突（如需放弃：git rebase --abort）
> git checkout dev # 切换到 dev 分支
> git rebase origin/dev # 用 rebase 同步远端 dev
> # 冲突处理和步骤 2 一致，先解决 dev 自身与远端的冲突，确保 dev 同步到远端最新
> git rebase main # 把 dev 变基到本地最新的 main 分支上
> ```
...
通过创建一个新的 “合并提交”（`Merge Commit`），将两个分支的修改历史连接起来，保留双方的完整提交记录。`Git` 会分析 `main` 和 `feature` 的共同祖先提交，计算两者的差异，然后生成一个新的合并提交，同时包含两个分支的所有修改。

```shell
# 假设您在分支 feature 开发, 需要将其合并到 main 分支
$ git checkout main
$ git merge feature # 级社合并过程中出现冲突
Auto-merging file.txt # Git 正在尝试自动合并 file.txt 文件
CONFLICT (content): Merge conflict in file.txt # file.txt 文件发生内容冲突
Automatic merge failed; fix conflicts and then commit the result. # 合并失败

$ git status
plaintext
You have unmerged paths. # 存在未合并的文件路径
  (fix conflicts and run "git commit") # 解决 "所有" 冲突后, 执行 git commit 可完成合并
  (use "git merge --abort" to abort the merge) # 可执行本指令来撤销当前合并过程

Unmerged paths: # 列出所有未合并的文件
  (use "git add <file>..." to mark resolution) # 当手动修改完冲突文件后需用本指令告知文件冲突已解决
        both modified:   file.txt
        
# 你查找所有需要修改的文件

$ git add --all
$ git commit -m "本次解决合并的日志"

```

从共同祖先开始，普通合并把所有的冲突文件都整合到一起，然后等您修改完后再一次 `commit` 完毕。

##### 5.5.2.1.变基合并

将当前分支的所有提交 “移植” 到目标分支的最新提交之后，形成一条线性的提交历史，**不创建新的合并提交**。

```shell
# 假设您在分支 feature 开发, 需要将其合并到 main 分支
$ git checkout feature
$ git rebase main
First, rewinding head to replay your work on top of it... # Git 正在 "回退头指针", 准备将当前分支 feature 的提交(也就是你的工作)重新 "播放(应用)" 到目标分支(main)的最新提交之上
Applying: D (提交信息：修改 file1.txt) # 当前正在尝试将 feature 分支的 D 提交(D 是提交的代号, 提交信息是 "修改 file1.txt") 应用到目标分支上
Auto-merging file1.txt # Git 尝试自动合并 file1.txt 文件
CONFLICT (content): Merge conflict in file1.txt # 冲突核心提示: file1.txt 发生了内容冲突(content 表示是代码内容不一致导致的冲突)
error: could not apply D... 修改 file1.txt # 操作失败提示: 由于冲突无法自动解决, D 提交暂时无法应用到目标分支上
# 冲突时的指引指令
hint: Resolve all conflicts manually, mark them as resolved with # 手动解决 file1.txt 中的冲突
hint: "git add/rm <conflicted_files>", then run "git rebase --continue". # 用 git add <冲突文件> (这里是 git add file1.txt) 标记冲突已解决, 然后执行 git rebase --continue 继续处理后续提交
hint: You can instead skip this commit with "git rebase --skip". # 如果不想要 D 提交的修改了, 可以用 git rebase --skip 跳过这个提交, 直接处理下一个提交(注意跳过会丢失 D 提交的内容)
hint: To abort and get back to the state before "git rebase", run "git rebase --abort". # 如果想终止整个变基过程, 回到执行 git rebase 之前的状态

# 而由于我们是把另外一个提交应用到 main 分支上所以无需重新使用 git commit (但是会自动跳出一个支持修改的 vim 界面, 允许你重新修改自己的 commit), 只需要按照指引 "解决冲突、标记解决、继续变基" 即可

# 后续想要查看哪些文件冲突可以使用 git status 然后继续变基(类似之前的普通合并), 直到所有 commit 都应用到分支上

# 当 commit 都应用上时, 就会提示 "Successfully rebased and updated refs/heads/feature."
```

从共同祖先开始，变基合并是不是把某个分支应用在另外一个分支上，一次一次应用另外一个分支的 `commit`，直到冲突解决，不断继续应用不断解决冲突。
