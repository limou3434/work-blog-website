<template><div><h2 id="_1-铺垫概念" tabindex="-1"><a class="header-anchor" href="#_1-铺垫概念"><span>1.铺垫概念</span></a></h2>
<p>理解守护进程之前，我们要先来理解一些周边概念：</p>
<ul>
<li>
<p><strong>PGID</strong>：实际上一个进程除了 <code v-pre>pid、ppid</code> 还有一个组 <code v-pre>ID</code> 也就是 <code v-pre>PGID</code>，组长一般是父进程，也就是第一个被分配 <code v-pre>pid</code> 进程（不过值得注意的是 <code v-pre>bash</code> 本身自成一组）</p>
</li>
<li>
<p><strong>SID</strong>：进程还有一个会话 <code v-pre>ID</code> 也就是 <code v-pre>SID</code>，任何进行登录的用户，需要有多个进程（组）来给该用户提供服务，以及用户自己启动的进程或服务，最终都会属于同一个 <code v-pre>SID</code></p>
</li>
<li>
<p><strong>PPID</strong>：在 <code v-pre>bash</code> 中同时用管道启动多个进程，则这些进程都是兄弟进程（都有共同的 <code v-pre>PPID</code>），父进程都是 <code v-pre>bash</code>，可以通过管道来通信</p>
</li>
<li>
<p><strong>前台进程</strong>：和终端相关联的进程就是前台进程，简单说就是能从终端正常获取输入并且正常输出的进程（<code v-pre>bash</code> 本身就是一种前台进程）。而若一个进程被 <code v-pre>bash</code> 运行起来后，用户无法继续和 <code v-pre>bash</code> 进行交互，则说明前台进程从 <code v-pre>bash</code> 切换到了该进程</p>
</li>
<li>
<p><strong>登录和注销</strong>：<code v-pre>Windows</code> 下实际上也有登录和退出登录的概念，那就是“注销”而非“关机”选项，一旦注销，就会把当前用户启动的相关进程或服务全部杀掉，也可以达到解决卡顿的目的。</p>
</li>
</ul>
<h2 id="_2-守护进程" tabindex="-1"><a class="header-anchor" href="#_2-守护进程"><span>2.守护进程</span></a></h2>
<Mermaid code="eJxLL0osyFAIceHiKi5NgnCUCoryk1OLi+PTi/JLC+INlbgUgMBRQykpsThDQVuhJLUoNzMvMUdJkys1LwW3RiOIRicNuLghUAtIyBkhZAQVckEIGRMyWE9PT4mL01VDCcSAqnXTUCoGqsjMzwOKuCno6topoHlDQQ1NxAhDBGgeFwAHElRj"></Mermaid><p>如果用户退出登录了，理论上上述会话内的进程和服务都需要被释放（大部分需要，不同版本的退出处理机制不一样）。</p>
<p>如果我们 <strong>让一个进程自成一个会话，就可以让该进程变成守护进程</strong>。而 <code v-pre>Linux</code> 就提供了 <code v-pre>&lt;unistd.h&gt;</code> 内的 <code v-pre>pid_tr setsid(void);</code> 系统调用，可以让一个进程变成会话。失败返回 <code v-pre>-1</code>，成功返回调用该函数的进程。</p>
<p>另外还需要保证调用进程不是进程组组长（组长不允许成为守护进程），可使用 <code v-pre>fork()</code> 创建的子进程来保证该进程不是组长。</p>
<blockquote>
<p>补充：而且 <code v-pre>Linux</code> 下还有关于守护进程的 <code v-pre>daemon()</code> 接口，可以让一个进程变成守护进程，但不怎么使用。</p>
</blockquote>
<p>下面我写一份代码，我先不运用这份代码，但是在之后的网络代码中会尝试进行运用，您可以先简单看看这里的代码：</p>
<div class="language-cpp line-numbers-mode" data-highlighter="shiki" data-ext="cpp" style="--shiki-light:#393a34;--shiki-dark:#dbd7caee;--shiki-light-bg:#ffffff;--shiki-dark-bg:#121212"><pre class="shiki shiki-themes vitesse-light vitesse-dark vp-code" v-pre=""><code class="language-cpp"><span class="line"><span style="--shiki-light:#A0ADA0;--shiki-dark:#758575DD">//daemon.hpp</span></span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>另外，我们也可以看到，守护进程实际上就是一种被托管的特殊的孤儿进程。</p>
<h2 id="_3-发布作业" tabindex="-1"><a class="header-anchor" href="#_3-发布作业"><span>3.发布作业</span></a></h2>
<p><code v-pre>at</code> 命令用于在指定时间内执行一次性的任务，例如下面用法</p>
<div class="language-bash line-numbers-mode" data-highlighter="shiki" data-ext="bash" style="--shiki-light:#393a34;--shiki-dark:#dbd7caee;--shiki-light-bg:#ffffff;--shiki-dark-bg:#121212"><pre class="shiki shiki-themes vitesse-light vitesse-dark vp-code" v-pre=""><code class="language-bash"><span class="line"><span style="--shiki-light:#59873A;--shiki-dark:#80A665">$</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> at</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 10:46</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> PM</span></span>
...
<span class="line"><span style="--shiki-light:#59873A;--shiki-dark:#80A665">job</span><span style="--shiki-light:#2F798A;--shiki-dark:#4C9A91"> 4</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> at</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> Wed</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> Apr</span><span style="--shiki-light:#2F798A;--shiki-dark:#4C9A91"> 24</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 22:46:00</span><span style="--shiki-light:#2F798A;--shiki-dark:#4C9A91"> 2024</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这将在本日的晚上 <code v-pre>10:46</code> 点执行任务。要删除作业，可以使用 <code v-pre>atrm</code> 命令，后跟作业的 <code v-pre>ID</code>。</p>
<p>还有一个 <code v-pre>crontab</code> 可以设置可重复执行的、周期性的作业。</p>
<ol>
<li>
<p>创建或编辑 <code v-pre>crontab</code> 文件：使用 <code v-pre>crontab -e</code> 命令来编辑当前用户的 <code v-pre>crontab</code> 文件。如果之前没有设置过 <code v-pre>crontab</code>，它会打开一个新的编辑器，并在其中添加或编辑 <code v-pre>cron</code> 作业。如果已经有了 <code v-pre>crontab</code>，它会打开一个编辑器，允许您编辑现有的 <code v-pre>cron</code> 作业。</p>
</li>
<li>
<p>添加 <code v-pre>cron</code> 作业：在编辑器中按照 <code v-pre>cron</code> 作业的格式添加要执行的命令和时间规范，格式如下：</p>
<div class="language-bash line-numbers-mode" data-highlighter="shiki" data-ext="bash" style="--shiki-light:#393a34;--shiki-dark:#dbd7caee;--shiki-light-bg:#ffffff;--shiki-dark-bg:#121212"><pre class="shiki shiki-themes vitesse-light vitesse-dark vp-code" v-pre=""><code class="language-bash"><span class="line"><span style="--shiki-light:#59873A;--shiki-dark:#80A665">分</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 时</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 日</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 月</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 周</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> 命令</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>其中，分表示分钟（<code v-pre>0-59</code>），时表示小时（<code v-pre>0-23</code>），日表示一个月中的某一天（<code v-pre>1-31</code>），月表示月份（<code v-pre>1-12</code>），周表示一周中的某一天（<code v-pre>0-6</code>，<code v-pre>0</code> 代表周日），使用 <code v-pre>*</code> 就代表该时间段内的任意一个时间点，命令是要执行的命令或脚本。</p>
<p>例如，要在每分钟执行一个脚本，可以这样添加一个 <code v-pre>cron</code> 作业：</p>
<div class="language-bash line-numbers-mode" data-highlighter="shiki" data-ext="bash" style="--shiki-light:#393a34;--shiki-dark:#dbd7caee;--shiki-light-bg:#ffffff;--shiki-dark-bg:#121212"><pre class="shiki shiki-themes vitesse-light vitesse-dark vp-code" v-pre=""><code class="language-bash"><span class="line"><span style="--shiki-light:#59873A;--shiki-dark:#80A665">0</span><span style="--shiki-light:#A65E2B;--shiki-dark:#C99076"> *</span><span style="--shiki-light:#A65E2B;--shiki-dark:#C99076"> *</span><span style="--shiki-light:#A65E2B;--shiki-dark:#C99076"> *</span><span style="--shiki-light:#A65E2B;--shiki-dark:#C99076"> *</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> echo</span><span style="--shiki-light:#B5695977;--shiki-dark:#C98A7D77"> "</span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D">hello</span><span style="--shiki-light:#B5695977;--shiki-dark:#C98A7D77">"</span><span style="--shiki-light:#AB5959;--shiki-dark:#CB7676"> >></span><span style="--shiki-light:#B56959;--shiki-dark:#C98A7D"> mytest</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div></li>
<li>
<p>保存和退出：编辑完成后，保存并退出编辑器。</p>
</li>
<li>
<p>列出当前的 <code v-pre>crontab</code> 文件：使用 <code v-pre>crontab -l</code> 命令列出当前用户的 <code v-pre>crontab</code> 文件中的所有作业。</p>
</li>
<li>
<p>删除 <code v-pre>crontab</code> 文件：使用 <code v-pre>crontab -r</code> 命令删除当前用户的 <code v-pre>crontab</code> 文件。</p>
</li>
</ol>
</div></template>
