...
1.   一般涉及到启动系统服务的操作都需要使用 `root` 权限，因此您最好提前切换到 `root` 账户。

2.   使用 `ps ajx | grep mysql 或 mariadb` 检查是否存在 `mysql或mariadb`（会显示一串目录），有就无需看后续的安装了。

3.   但是如果您想体验一下下载 `MySQL` 的流程，请使用 `systemctl stop mysql或mariadb.service` 指令停止 `mysql或mariadb` 服务。

4.   然后使用 `rpm -qa | grep mysql` 检查 `mysql或mariadb` 的安装包

5.   使用 `rpm -qa | grep mysql或mariadb | xargs yum -y remove` 卸载这些关于 `mysql` 的安装包，此时您的系统就成功卸载了 `MySQL或mariadb` 。
...
7.   然后使用 `yum install -y mysql-community-server` 一键安装 `MySQL` 即可

8.   使用 `systemctl start mysqld` 启动 `MySQL` 的服务端，使用 `ps` 可以查看是否启动成功（此时可以使用 `netstat -nltp` 来查看端口号）
...
# ./.env
# 认证配置
WORK_MYSQL_PASSWD=Qwe54188_

```

```yaml
# ./compose.yaml
# 如果需要快速把 docker run 指令转化为 docker compose 配置可以考虑使用这个插件 https://plugins.jetbrains.com/plugin/23108-docker-run-command-to-compose-converter
...
# 服务(服务名称、运行说明、镜像文件、容器名称、启动模式、集群路由、网络端口、工作目录、存储映射、环境变量)
services:
  work-mysql:
    image: mysql:8.0.41
    ...
    ports:
      - "3306:3306"
    volumes:
      - "./.data/work-mysql/memory/:/var/lib/mysql/"
      - "./.data/work-mysql/init/:/docker-entrypoint-initdb.d/"
    environment:
      TZ: Asia/Shanghai
      WORK_MYSQL_PASSWD: ${WORK_MYSQL_PASSWD}

```

然后运行指令 `sudo docker compose up -d work-mysql --build` 即可。

## 3.组件基础

### 3.1.登陆方式

当前我们需要在本地进行登陆才能使用 `MySQL`。

- 登录方法一：使用 `sudo grep 'temporary password' /var/log/mysqld.log` 获取临时 `root` 密码，较老的版本可以使用，新版本可能不行。然后使用 `mysql -u root -p` 输入临时密码即可，注意输入密码的时候不会回显。
- 登录方法二：直接使用 `mysql -u root -p` 登录
- 登录方法三：使用 `vim /etc/my.cnf` 打开 `MySQL` 的配置文件，然后在文件的最后添加上 `skip-grant-tables` 选项，保存退出（该做法就是设置为“跳过密码验证”）。后使用 `systemctl restart mysqld` 重启 `MySQL` 服务，然后使用 `mysql -u root -p` 即可直接登录 `MySQL`（可能还会显示输入密码，但是这次直接回车就可以）。
- 登录方式四：还有一种最为常用的登陆方式就是直接使用编程语言的某些库来进行访问，这也是一种常见的方式，不过我们后面再来提及。
...
使用 `MySQL` 之前，我们需要先了解一些概念，其中，我们安装的 `mysql` 和 `mysqld` 对应数据库的客户端和服务端。
...
>   [!WARNING]
>
>   注意：只是的数据库本身进行增删查改、备份、恢复等操作，而不是对数据库内的数据表做操作，还请您区分好这两点。
...
- 嵌套查询/子查询：在后面将会使用到，`SELECT` 还可以作为子句来嵌套查询（后面实践中会使用）。可以使用子查询进行单行子查询（单列），也可以多行子查询（多列）
...
- 多列子查询：单行和多行子查询都是针对一列来查询的，如果是多列则可以使用多列子查询
...
- 在 from 中使用子查询：`MySQL` 视一切为表，因此也可以将查询出来的表结构放在 `from` 的后面来检索。
...
[数据库设计简介_数据库设计那些事-慕课网](https://www.imooc.com/video/1903)，待补充...
...
而 `MySQL` 是可以移植的库，可以在任意操作系统上运行（主流场景是 `Linux`），对于一个 `MySQL` 服务端来说，主要有下面的模块组成：
