...
permalink: /1.编码修养/3.系统网络/1.基本操作/4uilxc2v/
---
...
管道是 `Linux` 中是原生提供的，作用很类似现实生活中的管道，都是进行“传输”的作用。不过这个管道只允许单向通行（也叫 **半双工**，这样设计比较简单。如果需要双向通信就需要创建两个管道文件，这样的通信就是 **全双工通信**）。为了保持进程之间的独立性，管道需要由操作系统（内核）来维护。
