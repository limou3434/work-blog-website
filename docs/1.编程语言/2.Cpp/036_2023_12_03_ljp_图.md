...
`Dijkstra` 算法每次都是选择 $Q\{...\} = V-S$ 中已确认最短的路径节点来进行更新（链接其他顶点），并加入 $S$ 中，所以该算法使用的是局部的贪心算法，时间复杂度为 $O(N^{2}),(N\text{是顶点个数})$。

这个算法还是蛮复杂的。

#### 3.4.2.贝尔曼-福特算法（Bellman-Ford）

`Dijkstra` 算法虽好，但是不支持负数权值的最短路径，而 `Bellman-Ford` 算法 [^4] 可以解决这一问题。该算法允许负权值的存在，并且思路相比 `Dijkstra` 算法更容易理解，但是效率会低下一些，时间复杂度为 $O(N · E),(N\text{为顶点数},E\text{为边数})$，而如果使用邻接矩阵实现的话时间复杂度还会降到 $O(N^{3}),(N\text{是顶点个数})$，我在本系列中依旧使用邻接矩阵来实现。
...
\begin{align}
ve(0) &= 0\\
ve(i) &= Max\{ve(k) + w_{k, i}\}, \text{k 为 i 的上一个顶点}
\end{align}
$$
...
>   总结：$\text{事件最早发生时间} = Max\{\text{前驱顶点的} ve() + \text{路径权值}\}$，期间将较小的顶点对删去，最后得到 $vl(i)$ 表的时候就可以直接得到关键路径，进而得到关键路径的长度，方便后续求解 $vl(i)$ 表，因为 $vl(i)$ 的最后一个就是关键路径的长度。
...
\begin{align}
vl(n-1) &= ve(n-1)\\
vl(i) &= Min\{vl(k) - w_{i, k}\}, \text{k 为 i 的下一个顶点}\\
\end{align}
$$
...
>   总结：$\text{事件最早发生时间} = Max\{\text{后继顶点的} vl() - \text{路径权值}\}$
...
$$
\begin{align}
e(i) &= ve(j), \text{j 为出度顶点}\\
\end{align}
$$


>   总结：$\text{活动最早开始时间} = \text{出度顶点的} ve()$
...
$$
\begin{align}
l(i) &= vl(k) - w_{j, k}, \text{k 为 } a_i \text{ 的边或弧头, j 为 } a_i \text{ 的边或弧尾}\\
\end{align}
$$

>   总结：$\text{活动最早开始时间} = \text{入度顶点的}vl() - \text{路径}$
